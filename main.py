...
def find_length_of_cable(distance: int, elektrodick: list):
    length: float = 0
    if len(elektrodick) <= 50:
        is_elements_in_limits: bool = True
        for bolt in elektrodick:
        ...
        if is_elements_in_limits:
            ...
            for bolt in elektrodick:
                index += 1
                if index != len(elektrodick):
                    if is_last_was_changed_to_one:
                        if elektrodick[index] > 1:
                            length += sqrt((elektrodick[index] - 1)**2 + distance**2)
                        else:
                            length += distance
                        is_last_was_changed_to_one = False
                    else:
                        with_no_changes: float = sqrt((elektrodick[index] - bolt)**2 + distance**2)
